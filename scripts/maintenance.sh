...
view_logs() {
    ...
    case $log_choice in
        ...
        4)
            ...
            ;;
        5)
            print_status "MySQL error logs (last 50 lines):"
            sudo tail -f -n 50 /var/log/mysql/error.log
            ;;
        6)
...
backup_database() {
    ...
    mysqldump -u root -p ppdb_production > $BACKUP_DIR/manual_backup_$DATE.sql
...
check_disk_space() {
    ...
    echo ""
    echo "=== Database Size ==="
    mysql -u root -p -e "SELECT table_schema 'Database', ROUND(SUM(data_length + index_length) / 1024 / 1024, 1) 'Size (MB)' FROM information_schema.tables WHERE table_schema='ppdb_production' GROUP BY table_schema;"
}
